...
Short Answer Worksheet - Part 1

This file contains the modified grid program for Worksheet Question #1:
* When the user clicks a cell, the clicked cell AND its neighbors update.
* Neighbor updates include: above, below, left, and right.
* Edge and corner clicks are safely handled with bounds checking.
        ...
        for event in pygame.event.get():
            ...
            # Mouse click event
                ...
                if row < 10 and column < 10:
                    ...
                    # Change the cell above (row - 1)
                    if row - 1 >= 0:
                        grid[row - 1][column] = 1

                    # Change the cell below (row + 1)
                    if row + 1 < 10:
                        grid[row + 1][column] = 1

                    # Change the cell to the left (column - 1)
                    if column - 1 >= 0:
                        grid[row][column - 1] = 1

                    # Change the cell to the right (column + 1)
                    if column + 1 < 10:
                        grid[row][column + 1] = 1
